...
class SLN_Helper_Intervals
{
    ...
    public function setDatetime(DateTime $date, $duration = null)
    {
	    ...
	    $suggestedTimeObject = new SLN_DateTime($suggestedTime);
	    $fromTimeObject      = new SLN_DateTime(reset($times));
	    $toTimeObject        = new SLN_DateTime(end($times));

	    if ($fromTimeObject > $suggestedTimeObject) {
		$suggestedTimeObject = $fromTimeObject;
	    } else if ($toTimeObject < $suggestedTimeObject) {
		$suggestedTimeObject = $toTimeObject;
	    } else {
		reset($times);
		foreach ($times as $time) {
		    $timeObject = new SLN_DateTime($time);
		    if ($suggestedTimeObject < $timeObject) {
			$suggestedTimeObject = $timeObject;
			break;
		    }
		}
	    }

	    $date->setTime(intval($suggestedTimeObject->format('H')), intval($suggestedTimeObject->format('i')));
        }
