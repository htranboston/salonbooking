...
final class Joy_Of_Text_Plugin_Messenger {
             ...
             function get_replace_tags($message,$member) {
                   ...
                   // Opt out
                   $optOutKeyword = '[opt-out]';
                   $optOutLink = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") .
                   "://$_SERVER[HTTP_HOST]" . "/wp-json/sms/v1/opt_out/" . $member['jot_grpmemid'];
                   $message = str_replace($optOutKeyword, $optOutLink, $message);

                   // Opt in
                   $optOutKeyword = '[opt-in]';
                   $optOutLink = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") .
                   "://$_SERVER[HTTP_HOST]" . "/wp-json/sms/v1/opt_in/" . $member['jot_grpmemid'];
                   $message = str_replace($optOutKeyword, $optOutLink, $message);
