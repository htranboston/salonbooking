	...
	static function detailed_list( $counts, $user_buckets, $nicetexts ) {
		...
		foreach ( array( 'member', 'guest', 'bot' ) as $user_type ) {
			...
			foreach ( $users as $user ) {
				...
				$output .= apply_filters("useronline_custom_template", "<div class='alert alert-secondary alert-dismissible'><strong>#$nr - $name</strong> $user_ip $_on $date<br/>$page_title<button type='button' class='close' data-dismiss='alert'>&times;</button></div>\n", $nr, $user);
			}
		}
