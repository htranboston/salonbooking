...
{
    const MAX_DAYS = 365;
    const TRANSIENT_GETDAYS_KEY = 'sln_helper_av_getdays';
    ...
    public function getCachedDays() {
        ...
        while ($count > 0) {
            ...
            if($day['status'] != 'booking_rules' && $day['status'] != 'holiday_rules') {
                $ret[] = $from->toString();
            }
    ...
    public function getTransientKey() {
        $avItems = $this->getItems();
        $hItems = $this->getHolidaysItemsWithWeekDayRules($avItems->getWeekDayRules());
        return self::TRANSIENT_GETDAYS_KEY . md5(serialize($avItems) . serialize($hItems));
    }
}
